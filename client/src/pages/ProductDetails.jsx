...
const ProductDetails = () => {
    ...
    const relatedProducts = useMemo(() => {
        if (products.length > 0 && product) {
            return products.filter((item) => product.category === item.category).slice(0, 5)
        }
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 mt-4">
                <div className="flex flex-col-reverse sm:flex-row gap-3 w-full lg:w-auto">
                    <div className="flex sm:flex-col gap-3 overflow-x-auto sm:overflow-visible">
                        {product.image.map((image, index) => (
                            <div key={index} onClick={() => setThumbnail(image)} className="border w-20 h-20 sm:w-24 sm:h-24 shrink-0 border-gray-500/30 rounded overflow-hidden cursor-pointer" >
                                <img src={image} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover" />
                            </div>
                        ))}
                    </div>

                    <div className="border border-gray-500/30 w-full max-w-[400px] aspect-square rounded overflow-hidden">
                ...
                <div className="text-sm w-full lg:w-1/2">
                    ...
                    <div className="flex flex-col sm:flex-row items-center mt-10 gap-3 sm:gap-4 text-base">
                        ...
                        <button onClick={() => { addToCart(product._id); navigate("/cart") }} className="w-full py-3.5 cursor-pointer font-medium bg-green-500 text-white hover:bg-primary-dull transition" >
                            Buy now
                        </button>
                    </div>
                </div>
